...
interface Report {
  ...
  verified: string;
  description: string;
  lattidude: number;  // note: typo in backend? 'lattidude' instead of latitude
  longidute: number;  // typo again? 'longidute' instead of longitude
  route_name: string;
  creator_id: number;
}
...
export default function MapPage() {
    ...
    fetch("http://217.153.167.103:8002/reports")
      .then(async (res) => {
        if (!res.ok) {
          ...
          console.error("Error loading reports:", res.status, err);
  ...
  // Handle marker click to fetch and show detailed info
  const onMarkerClick = async (reportId: number) => {
    const detailedReport = await fetchReportDetails(reportId);
    if (detailedReport) setSelectedReport(detailedReport);
  };
      ...
      <div className="absolute inset-0 z-0">
        <div className="h-full w-full">
          <MapWithNoSSR>
            ...
            {reports.map((report) => {
              const position: LatLngExpression = [report.lattidude, report.longidute];
              ...
              return (
                ...
                >
                  {/* Show popup only if this report is selected */}
                  {selectedReport && selectedReport.id === report.id && (
                    <Popup
                      position={position}
                    >
                      <div className="text-sm max-w-xs">
                        <h2 className="font-bold mb-1">{selectedReport.route_name}</h2>
                        <p className="mb-1">{selectedReport.description}</p>
                        <p>
                          <strong>Likes:</strong> {selectedReport.likes} <br />
                          <strong>Dislikes:</strong> {selectedReport.dislikes} <br />
                          <strong>Verified:</strong> {selectedReport.verified}
                        </p>
                      </div>
                    </Popup>
                  )}
...
// Don't forget your MapWithNoSSR component import as in your original code:
